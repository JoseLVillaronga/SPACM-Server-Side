...
$query="SELECT lp_id,lp.p_id,lo_id,lp_fecha,p.cli_usuario,lp.ps_id,ps_color,ps_nombre,pr_mac_docsis FROM lote_prueba AS lp,prueba_estado AS pe,prueba AS p,prueba_resultados_docsis AS prd WHERE lp.ps_id = pe.ps_id AND lp.p_id = p.p_id AND lp.lp_id = prd.pl_id AND p_habilitado = 1 AND lo_id BETWEEN 101 AND 140 AND lp_id NOT IN (SELECT pl_id FROM prueba_resultados_docsis WHERE pr_ot = 1) ORDER BY lo_id";
$res=(array)Db::listar($query);
require_once "header.php";
?>
    <main class="container" id="content" style="background-color: rgba(255,255,255,.4);">
        <h4 style="text-align: center;">Monitor Opers Virt - Upgrade</h4>
          ...
          <table class="striped centered blue lighten-5">
	        ...
	        foreach($res as $fila){ ?>
	          <tr class="resalta">
	            ...
	            <td><?php echo $fila['pr_mac_docsis']; ?></td>
