...
if($_SERVER['REQUEST_METHOD']=="POST"){
	...
	$res=Db::listarCDR("SELECT * FROM clientes WHERE cli_usuario = '".$usuario."' AND cli_password = '".Encripta::encrypt($password)."'");
	if(count($res)=="0"){
		$usu->loged=false;
	}else{
		$usu->loged=true;
...
?>
	<main class="container row" id="content" style="background-color: rgba(255,255,255,.4);">
		...
		<?php
		$res2=Db::listarCDR("SELECT * FROM clientes WHERE cli_usuario = '".$usuario."' AND cli_password = '".$password."'");
