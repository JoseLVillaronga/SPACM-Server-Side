...
class LoteStock
{
	 ...
	 * Metodo constructor ..
	 */
	public function __construct($c,$cache){
		$this->stockId=$c;
		if(empty($cache)){
		...
		if(empty($nS)){
			...
			if(in_array($this->artId, $check1)){
				if(ctype_xdigit($nS) AND strlen($nS)=="12"){
					...
					$queryS="SELECT lstk_serie FROM lote_stock,stock 
					WHERE lote_stock.stk_id = stock.stk_id 
					AND lstk_id NOT IN (SELECT lstk_id FROM lote_stock WHERE ss_id IN (4,5)) 
					AND stk_activo NOT IN (2) 
					AND lstk_serie IS NOT NULL
					AND lstk_serie = '".$nS."'";
					//$_SESSION['queryS']=$queryS;
					$check=Db::listarCDR($queryS);
					if(count($check)==0){
						$this->serie=$nS;
	...
	public function traePropiedadesPorId($nS,$cache){
		//$query="SELECT * FROM lote_stock WHERE stk_id = $nI AND lstk_serie = '$nS'";
		if(empty($cache)){
